...
import { listHidden } from './list.jsx';
...
const COMPONENTS = { list: ListView, day: DayView, week: WeekView, quarter: QuarterView };
let mounted = null;                       // which zoom Preact currently owns

export function isComponentView(z){ return !!COMPONENTS[z]; }

/* The host owns #view's contents, so it is where the drag engine is installed:
   every drag source is inside it, and the hook detaches with it. */
function ViewHost({ zoom }){
  const ref = useRef(null);
  useDrag(ref);
  const Component = COMPONENTS[zoom];
  return <div ref={ref} class="viewhost">{Component ? <Component /> : null}</div>;
}

function mountView(v, z){
  if (COMPONENTS[z]){
    /* No clearing here. Preact keeps its vnode tree on the container, so wiping
       innerHTML underneath it leaves that tree pointing at nodes which are no
       longer in the document — the next render then patches detached DOM and the
       view comes up empty. The host takes the zoom as a prop instead and swaps
       the child component itself, which is a diff Preact already knows how to do. */
    if (mounted === null) v.innerHTML = '';
    mounted = z;
    preactRender(<ViewHost zoom={z} />, v);
    return;
  }
  /* no string views left; kept as the seam for anything not yet converted */
  if (mounted){ preactRender(null, v); mounted = null; }
  v.innerHTML = '';
}
